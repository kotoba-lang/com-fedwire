(ns fedwire.main
  "Kotodama WASM entrypoint for the Fedwire clean-room actor (L4) — Clojure port.
  ...
  facts (`fedwire.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fedwire")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"fedwire.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fedwire-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fedwire-compat/ready)
